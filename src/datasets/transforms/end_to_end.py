...
class LoadFrames:
    ...
    def random_trunc(self, feats, trunc_len, gt_segments, gt_labels, offset=0, max_num_trials=200):
        ...
        gt_segments = gt_segments - st  # shift the time stamps due to truncation
        gt_labels = gt_labels[seg_idx]  # [N]
        return feats, gt_segments, gt_labels, st, trunc_len
    ...
    def __call__(self, results):
        ...
        offset_frames=max(-offset_frames, 0)
        offset_frames=self.offset_frames if self.offset_frames is not None else offset_frames
        frame_num =None # for padding
        frame_stride = None
        ...
        if self.method == "resize":
            ...
            self.trunc_len = results["resize_length"]
            ...
            frame_idxs = np.arange(offset_frames, total_frames, frame_stride)
            ...
            if trunc_start is None or trunc_len is None:
                frame_idxs, gt_segments, gt_labels, trunc_start, trunc_len = self.random_trunc(
                    frame_idxs,
                    trunc_len=frame_num,
                ...
                # compute the intersection between the sampled window and all segments
                window = np.repeat(window[None, :], num_segs, axis=0)
                left = np.maximum(window[:, 0], gt_segments[:, 0])
                right = np.minimum(window[:, 1] , gt_segments[:, 1])
                inter = np.clip(right - left, a_min=0, a_max=None)
                area_segs = np.abs(gt_segments[:, 1] - gt_segments[:, 0])
                inter_ratio = inter / area_segs
                # only select those segments over the thresh
                seg_idx = inter_ratio >= self.trunc_thresh
                frame_idxs = frame_idxs[st:ed]
                gt_segments = np.stack((left[seg_idx], right[seg_idx]), axis=1)  # [N,2] in feature grids
                gt_segments = gt_segments - st  # shift the time stamps due to truncation
            ...
            self.trunc_len = window_size * self.scale_factor
        elif self.method == "padding":
            raise NotImplementedError

        elif self.method == "all":
            if 'snippet_stride' in results:
                frame_stride = results["snippet_stride"] // self.scale_factor
            else:
                frame_num = results["resize_length"] * self.scale_factor
                frame_stride = total_frames / frame_num
            ...
            frame_idxs = np.arange(offset_frames, total_frames, frame_stride)
            results["gt_segments"] = results["gt_segments"] / self.scale_factor  # convert back to original scale
            results["trunc_start"] = results.get("trunc_start", None)
            results["trunc_len"] = results.get("trunc_len", None)

            frame_num = len(frame_idxs) // self.scale_factor
            self.trunc_len= frame_num

        # pad the frame_idxs
        if frame_num is not None and len(frame_idxs) < frame_num:
        ...
        results["masks"] = masks
        results['feat_fps']= fps * frame_stride if frame_stride is not None else fps
        return results
